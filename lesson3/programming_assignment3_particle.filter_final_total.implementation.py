...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        frontWheelAngle = random.gauss(float(motion[0]), self.steering_noise)
        ...
        turningAngle = (distance/float(self.length)) * tan(frontWheelAngle)
        

        if abs(turningAngle)>0.001:
        #bicycle model of the robot
            radius = distance/(turningAngle)
            
            Cx = x - sin(o) * radius
            Cy = y + cos(o) * radius

            x = Cx + sin(o+turningAngle)*radius
            y = Cy - cos(o+turningAngle)*radius
            o = (o + turningAngle)%(2*pi)
        else:
            x = x + distance*cos(o)
            y = y + distance*sin(o)
            o = o%(2*pi)
    ...
    # according to
    #           self.bearing_noise
    def sense(self, bearing_noise): #do not change the name of this function
        Z = []
...
def generate_ground_truth(motions):
    ...
    for t in range(T):
        ...
        Z.append(myrobot.sense(bearing_noise))
    #print 'Robot:    ', myrobot
    return [myrobot, Z]
