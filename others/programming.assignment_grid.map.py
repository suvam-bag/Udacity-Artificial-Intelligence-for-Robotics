...
def localize(colors,measurements,motions,sensor_right,p_move):
    ...
    for row in range(len(colors)):
        for col in range(len(colors[row])):
            p = p_move * p[[(row-motions) % len(colors)]((col-motions[row]) % len(colors[row]))]
            p = p + (1-p_move) * p[[(row) % len(colors)]((col) % len(colors))]
            hit  = (measurements == colors[row])
                    #q = [[]]
            p = (p[row] * (hit * sensor_right + (1-hit) * (1 - sensor_right)))
            s = sum(p)
            for row in range(len(colors)):
                for col in range(len(colors[row])):
                 p[row] = p[row] / s    
